...
def test_register(service: AuthService):
    registered_user, private_chat, auth = service.register(username='test', password='secret123')

    assert registered_user.username == 'test'
    assert registered_user.email is None
    assert registered_user.hashed_password == hash_password('secret123')
...
def test_double_register(service: AuthService):
    service.register(username='test', password='secret123')
...
def test_register_and_login(service: AuthService):
    ...
    login_user, login_auth = service.login(username='test', password='secret123')
...
def test_refresh_token(service: AuthService):
    user, _, auth = service.register(username='test', password='secret123')
...
def test_update_profile(service: AuthService):
    user, _, _ = service.register(username='test', password='secret123')
